...
export default function Hero() {
  ...
  // Role switching
  const roles = ["Full Stack .NET Developer", ".NET Developer", "Problem Solver"];
  const [index, setIndex] = useState(0);
  ...
  return (
    <section className="min-h-screen grid md:grid-cols-[40%_60%] items-center px-6 md:px-20 relative overflow-hidden">
      ...
      >
        ...
        <motion.h1 variants={item} className="text-3xl md:text-4xl font-bold leading-tight">
          Building Scalable & Secure Applications 🚀
        </motion.h1>

        <motion.p variants={item} className="text-xl mt-4 text-blue-400 font-semibold">
          {roles[index]}
        </motion.p>

        <motion.p variants={item} className="mt-6 text-gray-300 leading-7 max-w-xl">
          I am a passionate and detail-oriented Junior .NET Developer with nearly one year 
          of hands-on experience in building scalable and secure web applications using 
          ASP.NET Core, Blazor, and SQL Server.
          <br /><br />
          I have experience in developing RESTful APIs, implementing secure authentication 
          using JWT, optimizing database queries, and building responsive user interfaces. 
          I focus on writing clean, maintainable code and delivering efficient solutions 
          that enhance user experience and system performance.
          <br /><br />
          I enjoy solving real-world problems, learning new technologies, and continuously 
          improving my technical skills to build reliable and impactful applications.
        </motion.p>

        {/* Animated Tech Tags */}
        <motion.div variants={item} className="mt-6 flex flex-wrap gap-3">
          {["ASP.NET Core", "Blazor", "SQL Server", "JWT", "REST APIs"].map((tech, i) => (
            <motion.span
              key={i}
              whileHover={{ scale: 1.1 }}
              className="px-3 py-1 text-sm rounded-full bg-gray-800 text-gray-300 border border-gray-700 hover:bg-blue-600 hover:text-white transition"
            >
              {tech}
            </motion.span>
          ))}
